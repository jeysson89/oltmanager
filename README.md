...
### Очередь запросов (клиентская)
- Запросы к API устройства выполняются последовательно, чтобы избежать перегрузки.
- Появляется индикатор "Выполняется: ..." в правом нижнем углу.
- Окно исчезает после завершения всех операций.
- Разные вкладки/пользователи работают независимо.
